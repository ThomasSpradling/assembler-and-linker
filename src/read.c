...
int read_num(long int *output, const char *str, long int lower_bound, long int upper_bound) {
    ...
    int num = strtol(str, NULL, 0);
    if (num != 0L && num <= lower_bound && num <= upper_bound) {
        *output = num;
        return 0;
    }
    return -1;
...
// Converts assembly instructions into hex-code instruction formats.

/** `write_rtype` class commits an operation of the form:
 *     opcode   rs     rt     rd    shamt  funct
 *    [  6   ][  5  ][  5  ][  5  ][  5  ][  6   ]
*/
int write_rtype(int opcode, char *rs, char *rt, char *rd, char *shamt, int funct, FILE *output, size_t num_args, size_t expected_args) {
    ...
    int shamt_num;
    if (read_num((long *) &shamt_num, shamt, -(1<<4), (1<<4) - 1) == -1) {
        return 1;
    }

    uint32_t instruction = funct;
    instruction += shamt_num << 6;
 ...
 *    [  6   ][  5  ][  5  ][       16       ]
*/
int write_immediate_itype(int opcode, char *rs, char *rt, char *immediate, FILE *output, size_t num_args, size_t expected_args) {
    if (num_args != expected_args) {
        return 1;
    }
    int rs_num = read_register(rs);
    ...
    int immediate_num;
    if (read_num((long *) &immediate_num, immediate, -(1<<15), (1<<15) - 1) == -1) {
        return 1;
    };

    uint32_t instruction = immediate_num;
 ...
 *    [  6   ][  5  ][  5  ][       16       ]
*/
int write_branch_itype(int opcode, char *rs, char *rt, char *label, FILE *output, size_t num_args, size_t expected_args, SymbolTable *table, SymbolTable *relocate_table) {
    if (num_args != expected_args) {
        return 1;
    }
    int rs_num = read_register(rs);
    ...
    int64_t address_num = get_from_table(table, label);
    if (address_num < -(1<<15) || address_num > (1<<15) - 1) {
        return 1;
    }
    if (address_num == -1) {
        add_to_table(relocate_table, label, 0);
        address_num = 0;
    }

    uint32_t instruction = address_num;
    instruction += rt_num << 16;
    instruction += rs_num << (16 + 5);
    instruction += opcode << (16 + 5 + 5);
    write_instruction_hex(output, instruction);
 ...
 *    [  6   ][            26            ]
*/
int write_jtype(int opcode, char *label, FILE *output, size_t num_args, size_t expected_args, SymbolTable *table, SymbolTable *relocate_table) {
    if (num_args != expected_args) {
        return 1;
    }
    ...
    int64_t address_num = get_from_table(table, label);
    if (address_num < -(1<<25) || address_num > (1<<25) - 1) {
        return 1;
    }
    if (address_num == -1) {
        add_to_table(relocate_table, label, 0);
    ...
    uint32_t instruction = (uint32_t) address_num;
    instruction += opcode << 26;
    write_instruction_hex(output, instruction);
    return 0;
}

// Reads instruction in hexadecimal format
int read_instruction(FILE *output, const char *name, char **args, size_t num_args, uint32_t address,
  SymbolTable *table, SymbolTable *relocate_table) {
    ...
    else if (strcmp(name, "beq") == 0) return write_branch_itype(0x04, args[0], args[1], args[2], output, num_args, 3, table, relocate_table);

    // Branch on Not Equal: `bne $rs, $rt, label`
    else if (strcmp(name, "beq") == 0) return write_branch_itype(0x05, args[0], args[1], args[2], output, num_args, 3, table, relocate_table);

    // Jump: `j label`
    else if (strcmp(name, "j") == 0) return write_jtype(0x02, args[0], output, num_args, 1, table, relocate_table);

    // Jump and Link: `jal label`
    else if (strcmp(name, "jal") == 0) return write_jtype(0x03, args[0], output, num_args, 1, table, relocate_table);
